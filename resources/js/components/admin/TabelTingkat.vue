<template>
    <div>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div class="grid">
                <button @click="formToMakeTingkatOpen"
                ...
                <template v-for="(item, index) in items" :key="index">
                    <tr v-show="checkView(index + 1)"
                        ...
                        <th class="px-6 py-4 space-x-2 text-right">
                            <button @click="formToEditTingkatOpen(item.id)"
                            ...
                            <button @click="dialogToDeleteTingkatOpen(item.id)"
...
export default {
    ...
    emits: [
        "dataTingkat",
        "isFormToMakeTingkatTrue",
        "isFormToEditTingkatTrue",
        "isDialogToDeleteTingkatTrue"
    ],
    components: {ButtonExportDataSiswa},
    data() {
        return {
    ...
    methods: {
        loadDataTingkat() {
            axios.get("/api/admin/tingkat", {
                ...
                this.$emit('dataTingkat', data.data);
        ...
        formToMakeTingkatOpen(){
            this.$emit('isFormToMakeTingkatTrue', true);
        },
        formToEditTingkatOpen(id){
            const data = {
                id: id,
                status: true
            };
            this.$emit('isFormToEditTingkatTrue', data);
        },
        dialogToDeleteTingkatOpen(id){
            const data = {
                id: id,
                status: true
            };
            this.$emit('isDialogToDeleteTingkatTrue', data);
        }
    }
}
</script>

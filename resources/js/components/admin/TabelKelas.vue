<template>
    <div>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div class="grid">
                <button @click="formToMakeKelasOpen"
                ...
                <template v-for="(item, index) in items" :key="index">
                    <tr v-show="checkView(index + 1)"
                        ...
                        <th class="px-6 py-4 space-x-2 text-right">
                            <button @click="formToEditKelasOpen(item.id)"
                            ...
                            <button @click="dialogToDeleteKelasOpen(item.id)"
...
export default {
    ...
    emits: [
        "dataKelas",
        "isFormToMakeKelasTrue",
        "isFormToEditKelasTrue",
        "isDialogToDeleteKelasTrue"
    ],
    data() {
        return {
    ...
    methods: {
        loadDataKelas() {
            axios.get("/api/admin/kelas/tingkat-jurusan", {
                headers: {
        ...
        search(value) {
            if (value.length >= 1) {
                ...
                this.items = this.data.filter(item => {
                    return (
                        item.nama_kelas?.toLowerCase().includes(searchValue) ||
                        item.nama_tingkat?.toLowerCase().includes(searchValue) ||
                        item.nama_jurusan?.toLowerCase().includes(searchValue)
                    );
                });
            } else {
        ...
        formToMakeKelasOpen(){
            this.$emit('isFormToMakeKelasTrue', true);
        },
        formToEditKelasOpen(id){
            const data = {
                id: id,
                status: true
            };
            this.$emit('isFormToEditKelasTrue', data);
        },
        dialogToDeleteKelasOpen(id){
            const data = {
                id: id,
                status: true
            };
            this.$emit('isDialogToDeleteKelasTrue', data);
        }
    }
}
</script>

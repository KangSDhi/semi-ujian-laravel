<template>
    <div class="z-50 bg-slate-800 bg-opacity-50 flex justify-center items-center absolute top-0 right-0 bottom-0 left-0">
        <div class="bg-white dark:bg-gray-900 px-16 py-14 rounded-md">
            <h1 class="text-xl mb-4 font-bold dark:text-slate-200 text-slate-500">Buat Tingkat</h1>
            <div class="grid mb-2">
                <div>
                    <label class="block mb-2 text-sm font-medium dark:text-slate-300 text-slate-900">Nama
                        Tingkat</label>
                    <input v-model="dataCreate.nama_tingkat" type="text"
            ...
            <div class="flex justify-end gap-2">
                ...
                <button @click="formToMakeTingkatClose"
...
export default {
    ...
    methods: {
        createTingkat(){
            axios.post("/api/admin/tingkat/create", {
                tingkat: this.dataCreate.nama_tingkat
            }, {
                headers: {
        ...
        formToMakeTingkatClose(){
            this.$emit('isFormToMakeTingkatFalse', false);
        }
    }
}
</script>
